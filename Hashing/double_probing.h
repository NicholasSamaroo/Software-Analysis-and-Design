...
class HashTableDouble {
  ...
  */
  size_t FindPos(const HashedObj & x) const {
    ...
    size_t offset = secondInternalHash(x);
  ...
  size_t secondInternalHash(const HashedObj & x) const {
    static std::hash<HashedObj> hf;
    return (83 - ( hf(x) )  % 83) % array_.size();
    // Best prime number so far is 83
  }
